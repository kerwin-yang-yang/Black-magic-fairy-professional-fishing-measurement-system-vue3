...
import TableLog from "@/components/Tables/TableLog.vue";
...
<template>
    <LayoutAuthenticated>
        ...
        <SectionMain>
            <SectionTitleLineWithButton :icon="mdiTableBorder" title="捕获服务器" main>
            ...
            <Suspense>
                <template #default>
                    <CardBox class="mb-6" has-table v-if="isDataLoaded">
                        <TableLog checkable :data1="testdata" @showbutton="show" @editbutton="edit"
                            @deletebutton="deleteOne" :checkedRows="checkedOnes" @update-checked-rows="handleCheckedRows" />
                    </CardBox>
                </template>
                <template #fallback>
